...
namespace p2_ {
  ...
  void get_comp(std::pair<int, int> first, std::pair<int, int> second, std::pair<int, int> dimensions,
    ...
    while (true) {
      if(!check_valid_tennas(new_lhs, dimensions)) { break; }
      if (std::find(locations.begin(), locations.end(), new_lhs) == locations.end()) {
        locations.push_back(new_lhs); 
        /* std::cout << locations.size() << ": lhs; " << new_lhs.first << "," << new_lhs.second << std::endl;; */
      }
      new_lhs = {new_lhs.first + difference.first, new_lhs.second + difference.second};
    }
    std::pair<int, int> new_rhs = {second.first - difference.first, second.second - difference.second};
    while (true) { 
      if(!check_valid_tennas(new_rhs, dimensions)) { break; }
      if (std::find(locations.begin(), locations.end(), new_rhs) == locations.end()) {
        locations.push_back(new_rhs); 
        /* std::cout << locations.size() << ": rhs; " << new_rhs.first << "," << new_rhs.second << std::endl;; */
      }
      new_rhs = {new_rhs.first - difference.first, new_rhs.second - difference.second};
    }
  }
